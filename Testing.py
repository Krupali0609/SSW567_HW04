...
def get_repository_details(user_name):
    ...
    try:
        repository = requests.get(user_url)
    except (TypeError, KeyError, IndexError):
        return "Not able to fetch user's repos"

    repos = json.loads(repository.text)

    try:
        for repo in repos:
            repository_name = repo['name']
            repo_url = 'https://api.github.com/repos/{}/{}/commits'.format(user_name, repository_name)
            repo_detail = requests.get(repo_url)
            repo_detail_json = json.loads(repo_detail.text)
            result.append('Repository Name: {0} Number of commits in each Repository: {1}'.format(repository_name,len(repo_detail_json)))
    except (TypeError, KeyError, IndexError):
        return "Not able fetch user's commits"
